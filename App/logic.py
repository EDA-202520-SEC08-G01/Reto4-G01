...
def load_data(catalog, filename):
    ...
    with open(ruta, encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            try:
                ...
                dist_agua_m = 0.0
            dist_agua_km = dist_agua_m / 1000.0
    ...
    for i in range(total_eventos):
        ...
        if current_node is None:
            ...
            node["events_count"] = 1
            node["prom_distancia_agua"] = ev_dist_agua
            ...
            dt_hours = abs((ev_time - current_node["creation_timestamp"]).total_seconds()) / 3600.0
            d_km = haversine_km(
            ...
            if d_km < 3.0 and dt_hours < 3.0:
                ...
                current_node["prom_distancia_agua"] = (old_avg * (c - 1) + ev_dist_agua) / c
    ...
    for i in range(total_eventos):
        ...
        if node_id != prev_node_id:
            ...
            d_km = haversine_km(node_prev["lat"], node_prev["lon"],
    ...
    # Grafo migratorio
    keys_u = al.new_list()
    tmp = mp.key_set(dist_migratoria)
    for i in range(al.size(tmp)):
        al.add_last(keys_u, al.get_element(tmp, i))
    ...
    # Grafo hídrico
    keys_u = al.new_list()
    tmp2 = mp.key_set(dist_hidrica)
    for i in range(al.size(tmp2)):
        al.add_last(keys_u, al.get_element(tmp2, i))
